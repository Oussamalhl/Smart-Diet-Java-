...
public class CRUDProduit {
    ...
    public void ajouterProduits(Produit p){
        String req ="insert into produit (nom,description,quantite,prix,statut,categorie)"+"values (?,?,?,?,?,?)";
        try {
...
//            ste.setInt(6,p.getId_categories());
    ...
    ObservableList<Produit> list = FXCollections.observableArrayList();
        ...
        try {
             ...
             while (rs.next()) {   
              list.add(new Produit(rs.getInt("codeProduit"),rs.getString("nom"), rs.getString("description"),rs.getInt("quantite"),rs.getFloat("prix"),rs.getString("statut"),rs.getString("categorie")));
	...
	System.out.println("Deleted succefully");
     ...
     //Update produit in Data base
		   public boolean UpdateProduit(Produit p){
		          String sql = "UPDATE produit SET nom=?,description=?,quantite=?,prix=?,statut=?,categorie=?  WHERE codeProduit=?";
//		
		PreparedStatement statement;
		        try  
		        {
